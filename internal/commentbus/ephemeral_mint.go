...
var (
	daemonEphemeralHandleRE = regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9._-]*[.]e-[0-9a-f]{8}$`)
...
// ephemeral/<handle>.json, so the CLI's `comment ephemeral ensure` reuse path
// reads a daemon-minted credential identically to a CLI-minted one. During the
// rename rollout the daemon also writes a legacy ethereal/<handle>.json copy so
// older CLIs can still read back the one-time secret from their historical path.
...
// mintEphemeralViaPairing mints a session-scoped ephemeral handle for the paired
...
//
// The minted agent_secret is written straight to the ephemeral store (0600) and
...
// ephemeralPrepareStore creates the ephemeral store, hardens its mode, rejects a
...
func ephemeralPrepareStore(dir string) *SocketError {
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return socketError("INTERNAL_ERROR", "could not create ephemeral store", false)
	}
	...
	if lst, lerr := os.Lstat(dir); lerr != nil || lst.Mode()&os.ModeSymlink != 0 {
		return socketError("INTERNAL_ERROR", "ephemeral store is not a regular directory; refusing to write", false)
	}
	probe, perr := os.CreateTemp(dir, ".probe-*")
	if perr != nil {
		return socketError("INTERNAL_ERROR", "ephemeral store is not writable; refusing to mint", false)
	}
	name := probe.Name()
	_ = probe.Close()
	_ = os.Remove(name)
...
func (d *Daemon) mintEphemeralViaPairing(ctx context.Context, req SocketRequest) (map[string]any, *SocketError) {
	...
	// Prepare AND probe the credential stores BEFORE minting: the mint returns a
	...
	// quota). The legacy ethereal store is still prepared because pre-rename CLIs
	// read daemon-minted secrets back from that path and ignore the returned
	// cred_file. Fail here, before any mint is consumed.
	ephemeralDir := filepath.Join(d.paths.Home, "ephemeral")
	if sockErr := ephemeralPrepareStore(ephemeralDir); sockErr != nil {
		return nil, sockErr
	}
	legacyEphemeralDir := filepath.Join(d.paths.Home, "ethereal")
	if sockErr := ephemeralPrepareStore(legacyEphemeralDir); sockErr != nil {
		return nil, sockErr
	}
	...
	if !daemonEphemeralSecretRE.MatchString(minted.AgentSecret) ||
		!daemonEphemeralHandleRE.MatchString(minted.Handle) ||
	...
	credPath := filepath.Join(ephemeralDir, minted.Handle+".json")
	...
	legacyCred := cred
	legacyCred.IdentityClass = "ethereal"
	legacyData, err := json.Marshal(legacyCred)
	if err != nil {
		return nil, socketError("INTERNAL_ERROR", "could not encode legacy credential", false)
	}
	legacyCredPath := filepath.Join(legacyEphemeralDir, minted.Handle+".json")
	if err := WritePrivateFileAtomic(legacyCredPath, legacyData, 0o600); err != nil {
		return nil, socketError("INTERNAL_ERROR", "could not persist legacy credential", false)
	}
